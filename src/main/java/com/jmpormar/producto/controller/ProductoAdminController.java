...
public class ProductoAdminController {
    ...
    public ResponseEntity<ApiResponse<PageResponse<ProductoResponse>>> listar(
        ...
        return ResponseEntity.ok(ApiResponse.ok(
                service.listarAdmin(buscar, categoriaId, disponibilidad, activo, page, size)));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<ProductoResponse>> obtener(@PathVariable UUID id) {
        return ResponseEntity.ok(ApiResponse.ok(service.obtenerAdmin(id)));
    }

    @PostMapping
    public ResponseEntity<ApiResponse<ProductoResponse>> crear(@Valid @RequestBody ProductoRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.ok("Producto creado", service.crear(request)));
    }
    ...
    public ResponseEntity<ApiResponse<List<ProductoImagenResponse>>> ordenar(
        ...
        return ResponseEntity.ok(ApiResponse.ok("Galería reordenada", service.reordenar(id, request)));
    }
}
